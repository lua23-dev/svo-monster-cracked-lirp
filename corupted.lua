...
local function detectExecutor()
    ...
    if solara or getexecutorname and getexecutorname():lower():find("solara") then
        return "Solara"
    end
    
    if xeno or getexecutorname and getexecutorname():lower():find("xeno") then
        return "Xeno"
    end
    ...
    elseif identifyexecutor and identifyexecutor():lower():find("krnl") then
        return "Krnl"
    elseif fluxus and fluxus.getexecutorname then
        return "Fluxus"
    end
    return "Unknown"
end
...
print("Обнаружен экзекьютор:", executorName)
...
local function kickPlayer()
    ...
    if executorName == "Solara" then
        kickMessage = "Solara Detected: You've been terminated! [Code: 2]"
    elseif executorName == "Xeno" then
        kickMessage = "Xeno Detected: Access denied! [Code: 1]"
    elseif executorName == "Synapse" then
        kickMessage = "Synapse Detected: Session terminated!"
    elseif executorName == "Krnl" then
        kickMessage = "Krnl Detected: Security violation!"
    elseif executorName == "Fluxus" then
        kickMessage = "Fluxus Detected: Connection lost!"
    else
        kickMessage = "Unauthorized executor detected!"
    end
    
    
    print("[RAT GUI] Kicking with message: " .. kickMessage)
...
print("Экзекьютор: " .. executorName .. " - будет кик с соответствующим сообщением")
